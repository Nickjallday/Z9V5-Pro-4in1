...
void PrintJobRecovery::save(const bool force/*=false*/, const float zraise/*=0*/) {
  ...
  if (force
    ...
    info.flag.allow_cold_extrusion = TERN0(PREVENT_COLD_EXTRUSION, thermalManager.allow_cold_extrude);
		...
		MIXER_STEPPER_LOOP(i) { info.mixer_current_percent[i] =  mixer.mix[i]; }
		info.mixer_current_random_zpos[ZPOS_START] = mixer.random_mix.start_z; 
		info.mixer_current_random_zpos[ZPOS_END] = mixer.random_mix.end_z; 
		info.mixer_current_random_Height = mixer.random_mix.height;
		info.mixer_current_random_Extruders = mixer.random_mix.extruders;
		info.mixer_current_auto_zpos[ZPOS_START] = mixer.gradient.start_z;
  ...
  #if HAS_LCD_MENU
  	if (ui.IS_lcd_pause) 
		info.current_IS_lcd_pause = 1;
	else 
		info.current_IS_lcd_pause = 0;

	info.current_lcd_Pause_Zpos = ui.Lcd_pause_zpos;
  #endif
...
void PrintJobRecovery::resume() {
  ...
  #if HAS_DWIN_LCD && ENABLED(MIXING_EXTRUDER)
  	MixerCfg.Mixer_Mode_Rg = info.mixer_model;
  	if(MixerCfg.Mixer_Mode_Rg == 0){
  		mixer.selected_vtool = info.mixer_current_vtool;
  		...
  		mixer.gradient.start_z = info.mixer_current_auto_zpos[ZPOS_START];
  		mixer.gradient.end_z  = info.mixer_current_auto_zpos[ZPOS_END];
  		mixer.gradient.start_vtool = info.mixer_current_auto_vtool[VTOOL_START];
  		mixer.gradient.end_vtool = info.mixer_current_auto_vtool[VTOOL_END];
  		...
  		mixer.selected_vtool = info.mixer_current_vtool;
  		MixerCfg.Vtool_Backup = mixer.selected_vtool;
  		mixer.random_mix.start_z = info.mixer_current_random_zpos[ZPOS_START];
  		mixer.random_mix.end_z  = info.mixer_current_random_zpos[ZPOS_END];
		  mixer.random_mix.height = info.mixer_current_random_Height;
		  mixer.random_mix.extruders = info.mixer_current_random_Extruders;
  	}
  ...
  #if HAS_HOTEND
    HOTEND_LOOP() {
      const int16_t et0 = info.target_temperature[e];
      if (et0) {
        #if HAS_MULTI_HOTEND
          sprintf_P(cmd, PSTR("T%i S"), e);
          gcode.process_subcommands_now(cmd);
        #endif
        sprintf_P(cmd, PSTR("M109 S%i"), et0);
        gcode.process_subcommands_now(cmd);
      }
    }
  #endif
  ...
  if(!info.current_print_flag) {
  	//SERIAL_ECHOLNPAIR("info.current_position.z:\n",info.current_position.z);
 	dtostrf(info.current_position.z, 1, 3, str_1);
  	...
  	gcode.process_subcommands_now(cmd);
  		...
  		//SERIAL_ECHOLNPAIR("info.curret_Pause_Zpos_Buff:\n",info.curret_Pause_Zpos_Buff);
		dtostrf(info.current_Pause_Zpos_Buff, 1, 3, str_1);
  		#if Z_HOME_DIR > 0 || ENABLED(POWER_LOSS_RECOVER_ZHOME)
    		sprintf_P(cmd, PSTR("G1 Z%s F200"), str_1);
  		#else
