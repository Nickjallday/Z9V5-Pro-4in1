...
FORCE_INLINE void _draw_mixetruder_status() {	
	...
	if(mixer.gradient.enabled)
		...
		lcd_put_u8str(MIX_STATUS_X, MIX_STATUS_Y, "VTOOL:");
		const char *toolstr = i8tostr3rj(mixer.get_current_vtool());
		const uint8_t len = toolstr[0] != ' ' ? 3 : toolstr[1] != ' ' ? 2 : 1;
		lcd_put_u8str(MIX_STATUS_X+35, MIX_STATUS_Y, &toolstr[3-len]);
	}
}
...
void MarlinUI::draw_status_screen() {
  ...
  if (PAGE_UNDER(6 + 1 + 12 + 1 + 6 + 1)) {
  	// Mix Extrud
  	TERN_(DO_DRAW_MIXEXTRUDER, _draw_mixetruder_status());
