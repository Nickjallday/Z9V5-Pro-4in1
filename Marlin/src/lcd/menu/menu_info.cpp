...
//
  ...
  void menu_info_printer() {
    ...
    START_SCREEN();
	  ...
	  STATIC_ITEM_P(PSTR("TITAN Extruder"));
	...
	#if ENABLED(OPTION_TMC220X_XYZ)	  
	  #if ENABLED(OPTION_TMC220X_EXTRUDER)
		STATIC_ITEM_P(PSTR("TCM220X APPLIED ALL"));	
		#else
		STATIC_ITEM_P(PSTR("TCM220X APPLIED XYZ"));	
		#endif
  #endif
		...
		STATIC_ITEM_P(PSTR("TCM2225 APPLIED ALL"));	
	#else
		STATIC_ITEM_P(PSTR("TCM2225 APPLIED XYZ"));	
	#endif
#endif
	  ...
	  STATIC_ITEM_P(PSTR("DUAL Z ENDSTOP"));
  #endif
	
	#if ENABLED(OPTION_WIFI_MODULE)
	  STATIC_ITEM_P(PSTR("WiFi Feature"));
	#endif
