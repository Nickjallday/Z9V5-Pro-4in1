      ...
      for (PR_INNER_VAR = inStart; PR_INNER_VAR != inStop; pt_index++, PR_INNER_VAR += inInc) {
        ...
        TERN_(HAS_DISPLAY, ui.status_printf_P(0, PSTR(S_FMT " %i/%i"), GET_TEXT(MSG_PROBING_MESH), int(pt_index), int(abl_points)));
			  ...
			  TERN_(HAS_DWIN_LCD,DWIN_G29_Show_Messge(G29_MESH_PROBING,int(pt_index),int(abl_points)));
				...
				uint8_t m = 0;
				LOOP_L_N(k, 5){
					temp_z[k] = probe.probe_at_point(probePos, raise_after, verbose_level, true, true);
					if (verbose_level) SERIAL_ECHOLNPAIR("temp_z[k]= ", temp_z[k]);
					if (isnan(temp_z[k])){
						temp_z[k] = -99.9;
						TERN_(HAS_DWIN_LCD,DWIN_G29_Show_Messge(G29_MESH_VALUE,pt_index,abl_points,temp_z[k]));
						G29_RETURN(isnan(temp_z[k]));
					}
					else{
						TERN_(HAS_DWIN_LCD,DWIN_G29_Show_Messge(G29_MESH_VALUE,pt_index,abl_points,temp_z[k]));
						m++;
          ...
          z_values[meshCount.x][meshCount.y] = newz;
				  TERN_(HAS_DWIN_LCD,DWIN_G29_Show_Messge(G29_MESH_VALUE,pt_index,abl_points,z_values[meshCount.x][meshCount.y]));
