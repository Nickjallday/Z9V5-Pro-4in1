...
void GcodeSuite::M163() {
  ...
  #ifdef USE_PRECENT_MIXVALUE
	mixer.set_Percentmix(mix_index, (float)parser.intval('P'));
  ...
  void GcodeSuite::M165() {
    ...
    MIXER_STEPPER_LOOP(i) {
      if (parser.seenval(mixing_codes[i])) {
        SBI(mix_bits, i);		
	#ifdef USE_PRECENT_MIXVALUE
	    mixer.set_Percentmix(i, (float)parser.value_int());
        mixer.copy_mix_to_collector();
	#else
