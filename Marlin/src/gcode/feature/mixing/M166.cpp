...
inline void echo_mix() {
	  ...
	  SERIAL_ECHOPAIR(" (", int(mixer.mix[0]), "%|", int(mixer.mix[1]), "%|", int(mixer.mix[2]), "%)");
  #elif(MIXING_4_IN_1_OUT)
	  SERIAL_ECHOPAIR(" (", int(mixer.mix[0]), "%|", int(mixer.mix[1]), "%|",int(mixer.mix[2]), "%|",int(mixer.mix[3]), "%)");
  #endif
}
...
void GcodeSuite::M166() {
  ...
  TERN_(HAS_DWIN_LCD, HMI_flag.refersh_mix_flag = true);
